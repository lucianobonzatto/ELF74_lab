...
void timer0Callback(timer_callback_args_t *p_args) {
    (void) p_args;
    UINT msg = 11;
    tx_queue_send(&lcd_out_queue, &msg, TX_NO_WAIT);
}
...
void lcd_thread_entry(void) {
   ...
   gx_widget_attach(p_window_root, p_first_screen);
    ...
    while (1)
    {
        ...
        status = tx_queue_receive(&lcd_in_queue, msg, 10);
        while(status == 0){
            tratamento_mensagem_lcd_in(p_window_root->gx_widget_first_child, msg);
            status = tx_queue_receive(&lcd_in_queue, msg, 5);
            gx_system_canvas_refresh();
        }

//        msg_out = 11;
//        tx_queue_send(&lcd_out_queue, &msg_out, TX_WAIT_FOREVER);
        gx_system_canvas_refresh();
    }
}
/**********************************************************************************************************************
